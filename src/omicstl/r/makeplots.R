...
results <- results %>% 
  ...
  left_join(
    ...
    result %>% mutate(scenario = seq_len(nrow(.))), 
...
#            length(snr_target_vec), 
#			length(snr_source_vec), 
#			#length(prop_features_vec), 
#			length(response_fn_source_vec)))
...
for(param_idx in seq_len(nrow(params))) {
  ...
  mutate(
	model_id = case_when(
		model_id == "target" ~ "Weight-based TL",
		model_id == "target_nosource" ~ "No Transfer Learning",
		model_id == "target1" ~ "Variable Importance TL",
		model_id == "target2" ~ "Error Prediction TL",
		model_id == "target3" ~ "Error Augmentation TL"
	),
	model = case_when(
		model == "mult_mlp" ~ "Feed Forward\nNetwork",
		model == "mult_vae" ~ "Variational\nNetwork",
		model == "rf" ~ "Random-\nForest"
	)
  ) |>
  filter(model != "Random-\nForest")
  ...
  if(nrow(filtered_data) > 0) {
    p <- filtered_data %>%
	  ...
	  guides(color = guide_legend(title = "Training Strategy"))
    
    plot_name <- paste0(paste(
		metric, 
		snr_source_val, 
		snr_target_val, 
		#prop_features_val, 
		response_fn_source_val, sep = '_'), '.png')
    plot_path <- file.path(plots_dir, plot_name)
    
    cat(sprintf("  - Saving plot to: %s\n", plot_path))
    ggsave(plot_path, p, width = 14, height = 8, dpi = 300)
  } else {
...
for(param_idx in seq_len(nrow(params))) {
  ...
  if(nrow(filtered_data) > 0) {
    ...
    cat(sprintf("  - Saving plot to: %s\n", plot_path))
    ggsave(plot_path, p, width = 14, height = 8)
  } else {
    cat("  - WARNING: No data available for this parameter combination, skipping plot\n")
  }
}
